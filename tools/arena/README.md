...
- `task` (string, required): the viewer's task, in their own words. Free
  text — the only free-text field persisted anywhere by this app.
...
| `comparison` | `null`                 | `{ conditions, lowest_cost, lowest_turns }` | once, after all conditions finish; `conditions` mirrors the per-condition `metrics` above. `lowest_cost`/`lowest_turns` are each an **array** of the condition ids tied at the minimum (cost is the headline axis, not raw token count, since token classes are priced differently). Empty array when every condition errored, or when all candidates tie (a wash highlights nobody) |
